...
var FbAutocomplete = new Class({
	...
	completeAjax: function (r, v) {
		...
		if (r.length === 1) {
			...
			r.each(function (s) {
				this.cache['$' + s.text.toLowerCase() + '$'] = new Array(s);
			}.bind(this));
		}
	...
	menuPopulateOpen: function (data) {
		...
		if (data.length === 1 && this.options.autoLoadSingleResult) {
			v = this.getInputElement().get('value');
			r = data[0].text;
			i = r.toLowerCase().indexOf(v.toLowerCase());
			if (i >= 0)
			{
				ie = this.getInputElement();
				ie.value = r;
...
var FabCddAutocomplete = new Class({
	...
	search: function (e) {
		var key, observer, obsValue,
		v = this.getInputElement().get('value');
		if (v === '') {
			this.element.value = '';
		}
		if (v.toLowerCase() !== this.searchText && v !== '') {
			observer = document.id(this.options.observerid);
			...
			if (this.cache['$' + key + '$']) {
				...
				obsValue = document.id(this.options.observerid).get('value');
