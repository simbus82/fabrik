...
//***********************************************************/
	...
	showGroupError: function (r, d) {
		...
		if (this.tabbed) {
			currentTab = this.form.getElement('.tab-pane.active').id;
...
//console.log("c:",c);
	...
	duplicateResetElements: function (newElementControllers, group_id) {
		...
		newElementControllers.each(function (newEl) {
			...
			var pk_re = new RegExp(this.options.group_pk_ids[group_id]);
			var elName = newEl.element.name;
			if (!this.options.group_copy_element_values[group_id] || (this.options.group_copy_element_values[group_id] && elName && elName.test(pk_re))) {
